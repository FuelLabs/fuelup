...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let infra_repo_path = temp_dir.path().join("infra");
    Command::new("git")
        .args([
            "clone",
            &format!("https://github.com/{}.git", INFRA_REPO),
            infra_repo_path.to_str().unwrap(),
        ])
        .status()?;

    let temp_dir = tempdir()?;
    let compiler_explorer_path = temp_dir.path().join("compiler-explorer");
    Command::new("git")
        .args([
            "clone",
            &format!("https://github.com/{}.git", COMPILER_EXPLORER_REPO),
            compiler_explorer_path.to_str().unwrap(),
        ])
        .status()?;
