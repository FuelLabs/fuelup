...
The `latest` channel is possibly updated by a scheduled GitHub workflow that **runs every 30 minutes** and checks for new releases of `forc` and `fuel-core`.

### How the CI updates the `latest` channel

A channel is automatically updated if integration tests pass for the specific version of `forc` and `fuel-core`. This channel is typically found in a TOML file, and for the `latest` channel, is named `channel-fuel-latest.toml`.

### How the workflow works, a summary

The short explanation of how the scheduled workflow works is that we fetch all the latest versions after the latest published versions within `channel-fuel-latest.toml`, and we run tests for the possible combinations of them if not already marked as incompatible. After tests are done we either mark the version sets as incompatible if the tests failed or update the channel.

## Developer Guide

### Understanding the workflow

*Note: Reading the information below is only really necessary if you wish to contribute to the workflows or want a deeper understanding on how channels are updated*
