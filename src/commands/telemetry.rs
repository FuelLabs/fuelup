...
fn show_status() -> Result<()> {
    match get_telemetry_status()? {
        Some(TelemetryStatus::Enabled) => {
            info!("Telemetry is {}", colored_bold(ansiterm::Color::Green, "enabled"));
            info!("");
        ...
        Some(TelemetryStatus::Disabled) => {
            info!("Telemetry is {}", colored_bold(ansiterm::Color::Yellow, "disabled"));
            info!("");
            info!("Telemetry is currently disabled. No usage data will be collected.");
            info!("To enable, run: {}", bold("fuelup telemetry enable"));
        }
        None => {
            info!("Telemetry status is {}", colored_bold(ansiterm::Color::Yellow, "not set"));
            info!("");
            info!(
                "Telemetry preference has not been set. It will be disabled by default."
            );
            info!("To enable, run: {}", bold("fuelup telemetry enable"));
            info!("To disable, run: {}", bold("fuelup telemetry disable"));
        }
...
fn enable() -> Result<()> {
    ...
    info!("To disable at any time, run: {}", bold("fuelup telemetry disable"));
    Ok(())
}
...
fn disable() -> Result<()> {
    ...
    info!("To enable at any time, run: {}", bold("fuelup telemetry enable"));
    Ok(())
}
